...
class Trans4map_segformer(nn.Module):
    def __init__(self, cfg, device):
        ...
        print("mem_feat_dim:", mem_feat_dim.shape)
    ...
    def memory_update(self, features, proj_indices, masks_inliers):
        ...
        features = features.float() # torch.Size([1, 1, 64, 120, 160])
...
########################################################################################################################
        feature = features[:, 1, :, :, :].to(self.device)
        mask_inliers = masks_inliers[:, 1, :, :]  # torch.Size([1, 480, 640])
        proj_index = proj_indices[:, 1, :]  # torch.Size([1, 62500])
...
########################################################################################################################
        ...
        memory = memory.view(N, 250, 250, self.mem_feat_dim*2) # torch.Size([1, 250, 250, 256*2])
    ...
    def forward(self, rgb, proj_indices, masks_inliers):
        ...
        features = self.encoder(rgb) # torch.Size([1, 1, 3, 480, 640])
        features = features.unsqueeze(0) # torch.Size([1, 1, 64, 120, 160])

        # predictions = F.interpolate(predictions, size=(480,640), mode="bilinear", align_corners=True)
        ## memory updata is just
