...
from model.pano_data_loader import DatasetLoader_pano_detr
from lib2.dataset.dataset_matterport_sem_class20 import matterport_SemDataset33
from lib2.dataset.dataset_s2d3d_sem_class13 import S2d3dSemDataset
...
def train(rank, world_size, cfg):
    ...
    # dataset_dataset = "trans4map_pano"
    dataset_dataset = 'trans4map_pano_s2d3d'

    if dataset_dataset == 'matterport': 
    ...
    ### NEW MODEL WITH DEFORMABLE-DETR/home/zteng/anaconda3/envs/trans4map/lib/python3.8/site-packages/mmdet/models/utils/positional_encoding.py:81: UserWarning: __floordiv__ is deprecated, and its behavior will change in a future version of pytorch. It currently rounds toward 0 (like the 'trunc' function NOT 'floor'). This results in incorrect rounding for negative values. To keep the current behavior, use torch.div(a, b, rounding_mode='trunc'), or for actual floor division, use torch.div(a, b, rounding_mode='floor').

    # model = Trans4map_deformable_detr(cfg['model'], device)
    # model = Trans4pano_segformer(cfg['model'], device)
    model = proj_before_seformer(cfg['model'], device)
    ...
    for epoch in range(start_epoch, cfg["training"]["train_epoch"], 1):
        ...
        for batch in trainloader:
            ...
            rgb, rgb_no_norm, masks_inliers, proj_indices, semmap_gt, rotationz, map_mask, map_heights = batch

            # rgb, semmap_gt, fname= batch
            # print('semmap_gt:', torch.unique(semmap_gt), semmap_gt.size())
            ...
            semmap_gt = semmap_gt.float()
            semmap_gt = semmap_gt.unsqueeze(0)
            semmap_gt = F.interpolate(semmap_gt, size=(500, 500), mode="nearest")
            semmap_gt = semmap_gt.squeeze(0).short()
            # print('semmap_gt_0:', torch.unique(semmap_gt), semmap_gt.size())
        ...
        with torch.no_grad():
            for batch_val in valloader:
                ...
                semmap_gt = torchvision.transforms.ToPILImage()(semmap_gt)
                semmap_gt = torchvision.transforms.functional.rotate(semmap_gt, angle=degree, expand=False, center=None,
                                                                     fill=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        # default="model/model_cfg/model_pano_segformer.yml",
        default="model/model_cfg/model_proj_before_segformer.yml",
