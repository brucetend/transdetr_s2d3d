...
from Backbone.segformer import Segformer, mit_b0_kd, Segformer
...
from model.trans4pano_knowledge import Trans4map_knowledge


from model.loss import SemmapLoss
from model.loss import hcl_loss
from model.loss import hcl_loss_masked
from model.loss import KL_div_loss
...
def train(rank, world_size, cfg, teacher): 
    # Setup seeds
    ...
    #################################################### Setup Model ####################################################################################################
    model = Trans4map_knowledge(cfg['model'], device)
    ...
    if device.type == 'cuda':
        ...
        teacher = torch.nn.parallel.DistributedDataParallel(teacher, device_ids = [rank], find_unused_parameters=True)
    ...
    # hcl Loss
    loss_hcl = hcl_loss()
    loss_hcl = loss_hcl.to(device=device)


    # hcl Loss masked
    loss_hcl_masked = hcl_loss_masked()
    locc_hcl_masked = hcl_loss_masked().to(device=device)

    ## kl_div Loss
    loss_kl = KL_div_loss()
    loss_kl = loss_kl.to(device=device)

    ## embedding Loss
    # loss_embed = nn.MSELoss()
    ...
    for epoch in range(start_epoch, cfg["training"]["train_epoch"], 1):
        ...
        for batch in trainloader:
            ...
            semmap_pred, observed_masks, fstudent, fstudent_fuse = model(rgb, proj_indices, masks_inliers, rgb_no_norm)
            

            rgb_features = torch.nn.functional.interpolate(rgb, size=(512, 1024), mode = 'bilinear', align_corners=True)
            rgb_features = rgb_features.unsqueeze(0).to(device)
            
            # print('rgb_size:', rgb_features.size(), rgb.device) # [ 1, 1, 3, 1024, 2048]

            fteacher, fteacher_fuse  = teacher(rgb_features)
            # print('fteacher:', fteacher_fuse.size(), fstudent_fuse.size()) # torch.Size([1, 64, 256, 512])

            # embed_weights = [0.1, 0.1, 0.5, 1]
            # loss_embedding = 0
            # for i in range(len(estudent)):
            #     loss_embedding += embed_weights[i] * loss_embed(estudent[i], eteacher[i])    
            ...
            if observed_masks.any():
                ...
                if hcl_loss_ha == True:
                    # loss2 = loss_hcl(fstudent, fteacher)
                    loss2 = loss_hcl(fstudent, fteacher)
                    loss3 = loss_hcl(fstudent_fuse, fteacher_fuse)

                    # loss4 = loss_hcl(fstudent, fteacher)

                    loss = loss + 0.2 * loss2 + 0.001*loss3
                    # print('loss_loss:', loss,  0.2*loss2)
                else:
                    loss3 = loss_kl(fstudent, fteacher)
                    loss = loss + 0.2 * loss3
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="model/model_cfg/model_KD.yml",
    ...
    #### load_teacher
    teacher = Segformer_B4()
    # teacher = Segformer()
    # teacher_path = "./checkpoints/model_pano_segformer/2023-01-28-18-16-b4_segformer/ckpt_model.pkl"
    # teacher_path = "./checkpoints/model_pano_segformer/2023-02-02-23-07-B4/ckpt_model.pkl"
    teacher_path = "./checkpoints/model_pano_segformer/2023-02-08-22-29-B4-teacher/ckpt_model.pkl"
    # teacher_path = "./checkpoints/model_pano_segformer/2023-02-09-00-40-B2-crop-distortion/ckpt_model.pkl"

    # 这个teacher 很有可能是有问题的

    state_teacher = torch.load(teacher_path)
    weights_teacher = {}
    ...
    teacher.load_state_dict(weights_teacher, strict=False)
    ##############################################################################################################################################
    
    world_size = 4
    mp.spawn(train,
             args=(world_size, cfg, teacher),
             nprocs=world_size,
             join=True)
