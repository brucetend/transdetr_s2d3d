...
from model.s2d3d_pano_data_loader import s2d3d_DatasetLoader_pano
...
def train(rank, world_size, cfg):
    ...
    t_loader = s2d3d_DatasetLoader_pano(cfg["data"], split=cfg['data']['train_split'])
    v_loader = s2d3d_DatasetLoader_pano(cfg['data'], split=cfg["data"]["val_split"])
    t_sampler = DistributedSampler(t_loader)
    v_sampler = DistributedSampler(v_loader, shuffle=False)
    ...
    for epoch in range(start_epoch, cfg["training"]["train_epoch"], 1):
        ...
        for batch in trainloader:
            ...
            rgb, rgb_no_norm, masks_inliers, proj_indices, semmap_gt, rotationz = batch
            # print('rgb_batch:', rgb.size())
        ...
        with torch.no_grad():
            for batch_val in valloader:
                ...
                rgb, rgb_no_norm, masks_inliers, proj_indices, semmap_gt, rotationz = batch_val
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="model/model_cfg/model_pano.yml",
