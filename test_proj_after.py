...
from model.trans4pano_segformer import Trans4pano_segformer  ### with deformable attention block
from model.front_view_segformer import front_view_segformer  ### without deformable attention block
...
def memory_update(features, proj_indices, masks_inliers, device_haha):
    ...
    # print('features_size:', features.size()) # torch.Size([1, 20, 512, 1024])
...
# model = Trans4pano_segformer(cfg_model, device)  ### 前景分割模型！
model = front_view_segformer(cfg_model, device)
...
with torch.no_grad():
    for batch in testingloader:
        ...
        rgb, rgb_no_norm, masks_inliers, proj_indices, semmap_gt, rotationz ,_,_ = batch
        ...
        if observed_masks.any():
            ...
            print('pred:', pred.size(), pred)   # torch.Size([1, 14, 500, 500])
            ...
            assert gt.min() >= 0 and gt.max() < num_classes and semmap_pred.shape[3] == num_classes - 1
