...
class PerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            if self.use_cams_embeds:  # True
                ##print('cams_embeds:', self.cams_embeds[:,None, None,:].size(), feat.size())
                ### torch.Size([6, 1, 23200, 256], torch.Size([6, 1, 1, 256])
                feat = feat + self.cams_embeds[:, None, None, :].to(feat.dtype)
            feat = feat + self.level_embeds[None,
                                            None, lvl:lvl + 1, :].to(feat.dtype)
            spatial_shapes.append(spatial_shape)
            feat_flatten.append(feat)
            print('spatial_shapes_feat_flatten:', spatial_shapes, mlvl_feats[1].size())
        ...
        feat_flatten = feat_flatten.permute(
            0, 2, 1, 3)  # (num_cam, H*W, bs, embed_dims) (6, 30825, 1, 256)
    ...
    def forward(self,
        ...
        ## print('query_pos2:', query_pos.size(), query.size()) ### torch.Size([900, 256]) torch.Size([900, 256])
        ### object_query_embed 512维度，然后划分为query_pos, query 各256维

        query_pos = query_pos.unsqueeze(0).expand(bs, -1, -1)
        query = query.unsqueeze(0).expand(bs, -1, -1)
        ## torch.Size([1, 900, 256]) torch.Size([1, 900, 256])

        reference_points = self.reference_points(query_pos)
        reference_points = reference_points.sigmoid()
        ### 线性层 + sigmoid
        print('reference_points:', reference_points.size()) ### torch.Size([1, 900, 3])
